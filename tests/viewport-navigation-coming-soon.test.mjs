...
test('#launch-date is a full-section alias of #top', async () => {
  ...
  assert.match(html, /<span class="pm-viewport-alias" id="launch-date"[^>]*data-pm-viewport-target/);
  assert.match(html, /\.pm-viewport-alias\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;/);
  assert.doesNotMatch(html, /class="launch-card" id="launch-date"/);
});

test('#top and #launch-date produce identical alignment for identical section geometry', () => {
