...
const expectedLocales = ['en-US', 'nl-NL', 'zh-CN', 'hi-IN', 'es-ES', 'fr-FR', 'bn-BD', 'pt-BR', 'id-ID'];
const heldLocales = ['ar', 'ur-PK'];
const catalogLocales = [...expectedLocales, ...heldLocales];
...
test('language picker sits beside the logo and offers only this rollout\'s approved languages', async () => {
...
test('browser language detection and cookie override exclude held RTL locales', async () => {
  ...
  assert.equal(language.resolveLocale({ navigatorLanguages: ['nl-BE'] }), 'nl-NL');
  ...
  assert.equal(language.readLocaleCookie('theme=dark; reviewnudge_language=ur-PK'), '');
  assert.equal(language.resolveLocale({ navigatorLanguages: ['ar-SA', 'ur-PK'] }), 'en-US');
  assert.match(language.createLocaleCookie('pt-BR', { secure: true }), /Max-Age=31536000; Path=\/; SameSite=Lax; Secure$/);
  assert.equal(language.localeDefinition('ar').locale, 'en-US');
  assert.equal(language.localeDefinition('ur-PK').locale, 'en-US');
});

test('every active or held locale retains complete public, notification, and error catalogs', async () => {
  ...
  const folders = (await readdir(contentRoot, { withFileTypes: true }))
    ...
    .sort();
  assert.deepEqual(folders, [...catalogLocales].sort());
